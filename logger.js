...
const TEST = { "prop1":"val1", "prop2":1234, "prop3":{"sub1":"subval1", "sub2":false}, "prop4":[1,2,3,4,5,6,7,8,9,0] }

const objPP = ((isNode) => {
    if(isNode){
        return obj => { return `${util.inspect(obj, { colors: true })}` }
    }
    return obj => { return `${JSON.stringify(obj)}` }
})(NODE_RUNTIME);
...
const prettyJson = winston.format.printf(info => {
  if (info.message.constructor === Object) {
    info.message = NODE_RUNTIME ? util.inspect(info.message, { colors: true }) : JSON.stringify(info.message, null, 2)
  } else {
...
const logFormatConsole = winston.format.combine(
    ...
    winston.format.printf((info) => { info.level = info.level.toUpperCase(); return info }),
    winston.format.splat(),
...
// logger.debug('This is an object %j', TEST);
// logger.warn('This is the same object:', TEST);
// logger.warn(TEST);
// logger.error(TEST);
